...
impl<'a> UninitializedAccountInfo<'a> {
    ...
    /// Skip the checks required for an uninitialized account. This is useful for when another call
    /// will validate the account is empty and owned by the system program.
    #[inline(always)]
    pub fn new_unchecked(info: &'a AccountInfo) -> UninitializedAccountInfo<'a> {
        Self { info }
    }
}
