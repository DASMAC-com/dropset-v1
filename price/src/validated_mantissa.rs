...
const MAX_NORMALIZE_ITERATIONS: i32 = 100;
...
impl ValidatedPriceMantissa {
    ...
    /// Normalize an f64 to a validated price mantissa and the log_10 exponent used to get there.
    pub fn from_f64_with_normalize(
        nonnormalized_f64: f64,
    ) -> Option<(ValidatedPriceMantissa, i32)> {
...
mod tests {
    ...
    fn test_normalize_f64s() {
        ...
        assert!(matches!(normalize(1.32), Some((13_200_000, 7))));
        assert!(matches!(normalize(0.95123), Some((95_123_000, 8))));
        assert!(matches!(normalize(123_456_789.0), Some((12_345_678, -1))));
        assert!(matches!(normalize(78.12300001), Some((78_123_000, 6))));
        assert!(normalize(0.000000).is_none());
        assert!(normalize(0.0).is_none());
        assert!(normalize(0 as f64).is_none());
        assert!(normalize(-1.0).is_none());
        assert!(normalize(-0.0000000000001).is_none());

        let tiny_number = normalize(0.000_000_000_000_012_345_678);
        assert!(matches!(tiny_number, Some((12_345_678, 21))));
        ...
        assert!(
            matches!(tiny_imprecise_number, Some((10_000_000, 19)))
                || matches!(tiny_imprecise_number, Some((99_999_999, 20)))
        );
    }
}
